...
fn read_record(reader: & mut io::BufReader<File>) ->  Result<Option<WARCRecord>, std::io::Error>
{
    ...
    for line in reader.lines()
    {
        ...
        match kv.as_slice()
        {
            ...
            [ "" ]         => // end of header     
                {
                    let warc_version   = header.get("WARC").expect("Header Field WARC version missing.");
                    let warc_type      = match header.get("WARC-Type").expect("Header Field WARC-Type missing").as_ref()
                    {
                        ...
                        "continuation" => WARCType::Continuation,
                            ...
                            "response" => WARCType::Response,
                                 other => panic!(String::from("Not a known WARCType: ").push_str(other)),
                    };
                    ...
                    return Ok(Some(WARCRecord{warc_version : warc_version.clone(),
                                                 warc_type : warc_type,
...
//        .unwrap_or_else(|e| e.exit());
    ...
    let mut index_writer = index.writer(32293967295).expect("index writer failed");
    //let stdin = io::stdin();
    ...
    for path in std::fs::read_dir(warc_dir).unwrap()
    {
        ...
        while let Some(record) = read_record(& mut reader)?
        {
            match record.warc_type
            {
                ...
                WARCType::Conversion =>
                {
                    //eprintln!("{:?}", &record.warc_type);
                    //eprintln!("{:?}", &record.header);
                    //eprintln!("{:?}", &record.header.get("WARC-Target-URI"));
                    let body = std::str::from_utf8(&record.payload).expect("convert to utf8 failed");
                    count += 1;
                    if count % 1000 == 0
                    {
                        eprint!(".");
                        //println!("{}", record.content_length);
                    }
